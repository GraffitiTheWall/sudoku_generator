...
def draw_out_board(board) : 
    ...
    for row in board : 
        for char in row : 
            ...
            if char != 0 : 
                my_canvas.drawString(int((starting_x + starting_x + 25)/2) - 3 , int((starting_y + starting_y - 25) / 2) - 3 , str(char))
            my_canvas.setFont("Times-Roman" , 12)
            starting_x += 25
        starting_y -= 25
        starting_x = 15
    my_canvas.line(starting_x + (25 * len(board[0])) , 240 , starting_x + (25 * len(board[0])) , starting_y);
